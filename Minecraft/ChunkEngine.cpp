...
ChunkEngine::ChunkEngine(int w, int h, int d) : w(w), h(h), d(d) {
	volume = w * h * d;
...
voxel* ChunkEngine::get(int x, int y, int z) {
	int cx = x / CHUNK_W;
	int cy = y / CHUNK_H;
	int cz = z / CHUNK_D;
	if (x < 0) cx--;
...
Chunk* ChunkEngine::getChunk(int x, int y, int z) {
	if (x < 0 || y < 0 || z < 0 || x >= w || y >= h || z >= d)
		return nullptr;
	return chunks[(y * d + z) * w + x];
}

void ChunkEngine::set(int x, int y, int z, int id) {
	int cx = x / CHUNK_W;
	int cy = y / CHUNK_H;
	int cz = z / CHUNK_D;
	if (x < 0) cx--;
...
voxel* ChunkEngine::raycast(vec3 a, vec3 dir, float maxDist, vec3& end, vec3& norm, vec3& iend) {
	float px = a.x;
	float py = a.y;
	float pz = a.z;
